...
class Qreal {
  ...
  pass (key: string | number, data: any, parentObject: any, query: any): Promise<any> {
    return utils.async( _.get(this.middlewares, key).middlewares , ( middleware, _index, done ) => {
  ...
  use ( key : string,
    ...
    if ( _.has(this.middlewares, `${key}.middlewares`) ) {
      _.get(this.middlewares, key).middlewares.push(middleware)
    } else {
      // create space in middleware for data
      _.set(this.middlewares, `${key}.middlewares`, [ middleware ] )
    }
  
  }  

  run ( data: Array<any>, structure: Query | { [key: string]: Query }) {
    ...
    return utils.async(data, ( value, key, push ) => {
      ...
      utils.async( queries, ( query: Query | string, key, done ) => {
        ...
        const hadMiddlewares = ( _.has(this.middlewares, key) && !utils.isFalsy(context) ) ? _.get(this.middlewares, key) : false
  
        const subProcess = new Qreal();
        ...
        const restructure = ( data: any )=> {
          ...
          */
          ...
          if ( hadMiddlewares ) {
            ...
            subProcess.run(data, query as Query).then(( subObject: any[] ) => {
        ...
        if ( hadMiddlewares ) {
          ...
          if ( !hadMiddlewares.middlewares ) {
            subProcess.middlewares = { ..._.omit(hadMiddlewares, 'middlewares') }
            restructure( context )
            return
          }
  
          const pass = this.pass(key, context, { ...value, ...methods.$include(value, key) }, query)
  
          pass.then(( context: any[] ) => {
  
            if ( _.keys( hadMiddlewares ).length !== 1 && !!hadMiddlewares ) {
              subProcess.middlewares = {
